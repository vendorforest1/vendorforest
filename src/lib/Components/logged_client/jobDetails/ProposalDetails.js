    ...
    render() {
        ...
        const generateQA = () => {
            return this.props.proposal.answers.map((answer, index) => {
                return <div className="anwer-item mb-3" key={index}>
                    <h6 className="question text-grey mb-2">{index + 1}. {answer.question}</h6>
                    <p>{answer.answer}</p>
                </div>
            })
        }
                        ...
                        <div className="vendor-content mb-3">
                            <div className="row">
                                <div className="col-md-7 vendor-profile d-flex mb-2">
                                    <Avatar src={this.props.proposal.vendor.profileImage || defaultProfileImage} shape="square" size={80}/>
                                    <div className="ml-2">
                                        <h6 className="text-dark font-weight-bold">
                                            {
                                                this.props.proposal.vendor.firstName && this.props.proposal.vendor.lastName ? 
                                                `${this.props.proposal.vendor.firstName} ${this.props.proposal.vendor.lastName}` : this.props.proposal.vendor.username
                                            }
                                        </h6>
                                        <p>
                                            {
                                                this.props.proposal.vendor.vendor ? 
                                                `${this.props.proposal.vendor.vendor.service.name}/${this.props.proposal.vendor.vendor.category.name}` : 'NONE'
                                            }
                                        </p>
                                ...
                                <div className="col-md-5 vendor-status">
                                    <Progress percent={this.props.proposal.vendor.vendor.successRate} size="small" status="active" className="job-progress"/>
                                    <Rate disabled value={this.props.proposal.vendor.vendor.rate} allowHalf={true} className="like-rate"/>
                        ...
                        <div className="proposal-content">
                            <div className="price d-flex justify-content-around">
                                <div className="bid-price">
                                    ...
                                    <h4>${this.props.proposal.offerBudget}<small>{this.props.proposal.offerBudgetType === constants.BUDGET_TYPE.FIXED ? '' : '/hr'}</small></h4>
                                </div>
                                <div className="bid-price mb-2">
                                    <p className="text-grey">Job Price</p>
                                    <h4>${this.props.job.budget}<small>{this.props.job.budgetType === constants.BUDGET_TYPE.FIXED ? '' : '/hr'}</small></h4>
                                </div>
                            </div>
                            <hr/>
                            <div className="qa">
